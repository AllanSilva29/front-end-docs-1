...
Isso é a **mesma lógica** de:

```ts
const [usuario, setUsuario] = useState<User | null>(null);
```

Aqui, o `useState` é uma função genérica, e estamos dizendo: "o tipo do estado pode ser `User` ou `null`".
...
<Aside type="tip" title="Onde mais usamos funções genéricas? 🧩">

* **Funções utilitárias**: como `Array.map`, `Promise`, `fetch`, `filter`, etc.
* **Classes genéricas**: ex: uma `Queue<T>` para estrutura de dados.
* **Componentes genéricos**: em bibliotecas como React, Angular, Vue com TS, etc.
* **APIs com tipos dinâmicos**: como funções que recebem dados de um endpoint e precisam retornar um tipo baseado no parâmetro.
...
<Aside type="tip">
Veja mais sobre Genéricos na [documentação oficial do Typescript](https://www.typescriptlang.org/docs/handbook/2/generics.html)
</Aside>
